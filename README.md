...
GNU Graphics, idraw, ivtools, LaTex and Ghostscript can be
combined for desktop publishing. For example, one may generate
figures using plot2ps, edit them using idraw, include the figures
in LaTeX documents, generate printer-ready final postscript using
LaTeX, and preview the final output using GhostScript.

The `dvips` utility converts dvi files generated by LaTeX into
post-script.  It also has support for inclusion of PostScript
figures into LaTeX documents. It is availble via `apt get
