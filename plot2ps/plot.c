...
struct option	long_options[] = {
    ...
    { "prologue",	ARG_REQUIRED,	NULL, 'p' },
    { "line-width",	ARG_REQUIRED,	NULL, 'L' },
     ...
     char *argv[];
{
  ...
  while ((option = getopt_long(argc, argv, "-CF:HL:VWf:hlp:su", long_options, &opt_index)) != EOF) {
      if (option == 0)
	...
	case 'F':
	  ...
	  break;
	case 'L':		/* set the line width */
	  line_width = atoi (argv[i+1]);
	  break;
	case 'p':		/* use the users prolog */
	  user_has_prologue = 1;
	  users_prologue = argv[i+1];
	  break;
